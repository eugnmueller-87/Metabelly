...
def _result(
    category: Category, priority: Priority, language: Language = Language.HR
) -> TriageResult:
    return TriageResult(
        category=category,
        priority=priority,
...
class TestRouteChannel:
    ...
    def test_medical_goes_to_medical(self) -> None:
        assert (
            _route_channel(_result(Category.MEDICAL, Priority.P1_URGENT)) == _CHANNEL_MAP["medical"]
        )
    ...
    def test_business_goes_to_business(self) -> None:
        assert (
            _route_channel(_result(Category.BUSINESS, Priority.P2_BUSINESS))
            == _CHANNEL_MAP["business"]
        )

    def test_order_goes_to_orders(self) -> None:
        assert (
            _route_channel(_result(Category.ORDER, Priority.P3_SUPPORT)) == _CHANNEL_MAP["orders"]
        )
...
class TestSlackNotifier:
    def test_post_ticket_calls_slack(
    ...
    def test_slack_error_returns_false(
        self, mock_client: MagicMock, mock_settings: MagicMock
    ) -> None:
